...
export interface IncrementalSessionRuntimeProjector {
  readonly lastEventId: string | undefined;
  readonly lastSequence: number;
  append(events: RuntimeEvent[]): SessionRuntimeProjection;
}
...
export function createSessionRuntimeProjector(): IncrementalSessionRuntimeProjector {
  ...
  function append(events: RuntimeEvent[]): SessionRuntimeProjection {
    for (const event of events) {
      if (event.sequence !== lastSequence + 1) {
        throw new Error(`Non-contiguous Runtime Event projection at sequence ${event.sequence}`);
      }
...
export function projectSessionRuntime(events: RuntimeEvent[]): SessionRuntimeProjection {
  return createSessionRuntimeProjector().append(events);
}
