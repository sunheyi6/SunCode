...
export function buildSystemPrompt(input: SystemPromptInput): string {
  const { workingDir, tools, skillsContent, maxTurns, customPrompt } = input;
  ...
  parts.push('You have access to the following tools. Use them to explore, modify, and execute code.');
  parts.push('');
  ...
  parts.push('1. You may call multiple tools in a single response when operations are independent.');
  ...
  parts.push('4. When executing bash commands, include clear descriptions of what each command does.');
  ...
  // Final instruction
  parts.push('');
  parts.push('Begin by analyzing the user\'s request carefully. Use tools to gather information before proposing or making changes.');
