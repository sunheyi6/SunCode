...
<template>
  <details class="command-operation" :class="{ 'command-failed': isFailed }">
    <summary class="command-summary">
      <span class="command-icon">▸</span>
      <span class="command-title">{{ title }}</span>
      <span class="command-status">{{ isFailed ? '失败' : '完成' }}</span>
      <span class="command-exit-code">退出码：{{ exitCodeLabel }}</span>
    </summary>
    <div class="command-body">
      ...
      <section v-if="details?.stderr" class="command-section">
        <h4>错误输出</h4>
